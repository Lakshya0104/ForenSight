...
def print_os_profile(result: dict):
    ...
    left = Panel(
        ...
        f"[dim]Threat         [/dim] [{RISK_COLORS.get(threat, 'white')}]{threat}"
        f"[/{RISK_COLORS.get(threat, 'white')}]\n"
        f"[dim]Offensive Pkgs [/dim] [{'red' if pkg_count > 0 else 'green'}]"
        f"{pkg_count} found[/{'red' if pkg_count > 0 else 'green'}]",
...
def print_tools(result: dict):
    ...
    if not tools:
        console.print(Panel(
            "[green]No offensive tools detected.[/green]",
    ...
    for group, label, color in [
        (installed, "INSTALLED — Active Threats", "red"),
    ...
    console.print(Columns([
        Panel(cat_table, title="[dim]Category Breakdown[/dim]",
...
def print_evasion_score(result: dict):
    ...
    for key, pts in breakdown.items():
        ...
        contrib_bar      = (f"[{sig_color}]{'▮' * contrib_filled}[/{sig_color}]"
                            f"[dim]{'▯' * (12 - contrib_filled)}[/dim]")
        status           = (f"[{sig_color}]TRIGGERED[/{sig_color}]"
                            if pts > 0 else "[dim green]CLEAN[/dim green]")
    ...
    console.print(t)
    console.print(
        f"  [dim]Log Coverage:[/dim] [bold]{coverage:.0%}[/bold]   "
...
def print_mitre(result: dict):
    ...
    console.print(Panel(mt,
        title=f"[bold yellow]ATT&CK Techniques — {len(chain)} mapped "
              f"across {len(hit_phases)} phases[/bold yellow]",
        border_style="yellow"))
    console.print()
...
def print_persona(result: dict):
    ...
    conf_bar = (f"[{conf_color}]{'█' * conf_bar_filled}[/{conf_color}]"
                f"[dim]{'░' * (20 - conf_bar_filled)}[/dim]")

    console.print(Panel(
        f"[bold cyan]{persona}[/bold cyan]\n\n"
        f"[dim]{PERSONA_DESC.get(persona, '')}[/dim]\n\n"
        f"Confidence  {conf_bar}  [{conf_color}]{conf:.0%}[/{conf_color}]",
        title="[bold]Attacker Persona Classification[/bold]",
...
def print_uefi(result: dict):
    ...
    left = Panel(
        ...
        f"[bold {color}]{'⚠  DETECTED' if usb_sig else '✓  NOT FOUND'}[/bold {color}]\n\n"
    ...
    entries_text = ("\n".join(f"[dim]▸[/dim] {e}" for e in entries)
                    if entries else "[dim]No boot entries found[/dim]")
    right = Panel(
        entries_text + (
...
def print_narrative(result: dict):
    ...
    plain = result.get("narrative_plain", "")

    console.print(Panel(
        Padding(f"[italic]{tech}[/italic]", (1, 2)),
        title=f"[bold]Technical Report[/bold]  [dim]·[/dim]  {source_label}",
        border_style="cyan",
        subtitle="[dim]For forensic analysts and cybersecurity experts[/dim]"
    ))
    console.print()
    console.print(Panel(
        Padding(f"[italic]{plain}[/italic]", (1, 2)),
        title="[bold]Plain Language Report[/bold]",
        border_style="white",
        subtitle="[dim]For investigators, legal teams, and non-technical stakeholders[/dim]"
    ))
    console.print()
...
def print_verdict(result: dict, elapsed: float):
    ...
    active = len([t for t in tools if t["state"] == "installed"])
    ghost  = len([t for t in tools if t["state"] == "removed"])
    ...
    console.print(Panel(
        Align.center(
            ...
            f"([red]{active} active[/red] · [yellow]{ghost} ghost[/yellow])\n"
...
def generate_verdict(score: int) -> str:
    if score >= 80:
        return "HIGH RISK — Structured attack campaign detected. Evidence tampering confirmed. Immediate escalation recommended."
    if score >= 60:
        return "HIGH-MEDIUM RISK — Multiple evasion indicators found. Attacker attempted to cover tracks."
    if score >= 40:
        return "MEDIUM RISK — Suspicious activity detected. Further forensic investigation recommended."
    if score >= 20:
        return "LOW-MEDIUM RISK — Some anomalies found. May be benign or early-stage activity."
